...
function SkillTag({ skill, theme, onRemove }) {
  ...
  return (
    <div className="inline-flex items-center gap-2 bg-white border border-gray-200 rounded-full pl-3 pr-2 py-1.5 shadow-sm">
      ...
      {skill.level && (
        <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${levelColors[skill.level] || levelColors.beginner}`}>
          {skill.level}
        </span>
      )}
...
export default function SkillsSection({ profile, isCandidate, onSave }) {
  ...
  const handleAdd = () => {
    ...
    setSkills(prev => [...prev, { name: trimmed, level: newLevel }]);
    setNewSkill('');
    setError('');
    setShowInput(false);
  };
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-md">
      ...
      {/* Add skill input — shown when + Add Skill clicked */}
      {showInput && (
