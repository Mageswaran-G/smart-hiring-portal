...
export default function CertificationsSection({ profile, isCandidate, onSave }) {
  ...
  const handleCancel = () => { setShowForm(false); setForm(EMPTY); setError(''); setEditIndex(null); };
    ...
    <div className="bg-white rounded-2xl p-6 shadow-md">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{list.length} certificate{list.length !== 1 ? 's' : ''} added</p>
        </div>
        <button onClick={handleAdd} className={`inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border cursor-pointer ${theme.buttonLight}`}>
          <Plus size={13} />
          Add
        </button>
      </div>
      ...
      {list.length === 0 && !showForm ? (
        <div className="bg-gray-50 rounded-xl p-6 text-center">
          ...
          <p className="text-xs text-gray-300 mt-1">Add certifications to stand out to recruiters.</p>
        </div>
      ) : (
      ...
      {showForm && (
        <div className="border border-gray-200 rounded-xl p-4 mt-2">
          ...
          <div className="flex flex-col gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Certificate Name</label>
              <input name="name" value={form.name} onChange={handleChange} placeholder="AWS Certified Developer" className={inputClass} autoFocus />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Issuing Organization</label>
                <input name="issuer" value={form.issuer} onChange={handleChange} placeholder="Amazon Web Services" className={inputClass} />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Year</label>
                <input name="year" value={form.year} onChange={handleChange} placeholder="2024" className={inputClass} />
              </div>
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-400 mb-1 uppercase">Certificate URL</label>
              <input name="url" value={form.url} onChange={handleChange} placeholder="https://credential-link.com" className={inputClass} />
            </div>
            ...
            <div className="flex gap-2 justify-end mt-1">
              ...
              <button onClick={handleSubmit} disabled={saving} className={`px-4 py-2 text-sm text-white rounded-lg border-none cursor-pointer ${theme.button}`}>
